...
bool RtcClock::setTime(time_t time, uint16_t subseconds)
{
	...
	// set calendar
	LL_RTC_DATE_Config(RTC, currentTime.tm_wday, currentTime.tm_mday, currentTime.tm_mon, currentTime.tm_year);
	LL_RTC_DisableInitMode(RTC);
	// synchronize subseconds
	int32_t difference = temp_subsecond - subseconds;
	if(difference < 0)
	{
		LL_RTC_TIME_Synchronize(RTC, LL_RTC_SHIFT_SECOND_ADVANCE, LL_RTC_GetSynchPrescaler(RTC) + difference);
	}
	else if(difference > 0)
	{
		LL_RTC_TIME_Synchronize(RTC, LL_RTC_SHIFT_SECOND_DELAY, LL_RTC_GetSynchPrescaler(RTC) + difference);
	}
